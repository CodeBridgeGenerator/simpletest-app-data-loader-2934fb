...
const _setLoginEmail = async (email, access) => {
  const getUserLogin = await client
    .service("userLogin")
    .find({ query: { loginEmail: email } });
  if (getUserLogin?.data?.length === 0) {
    await client
      .service("userLogin")
      .create({ loginEmail: email, status: true, access: access });
  } else {
...
const _setLogOutEmail = async (email) => {
  ...
  if (getUserLogin?.data?.length === 0) {
    await client
      .service("userLogin")
      .create({ loginEmail: email, status: false, access: null });
  } else {
    const id = getUserLogin?.data[0]?._id;
