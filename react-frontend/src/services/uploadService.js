...
const UploadService = ({
  ...
  const FieldDataViewer = (results) => {
    ...
    setItems(sheets);
    setResults(results);
    if (results[items[activeIndex]?.label]?.data?.hasNot?.length > 0) {
      // console.log(results[items[activeIndex]?.label]?.data?.hasNot);
      const fields = Object.keys(
        results[items[activeIndex]?.label]?.data?.hasNot[0]
      );
      // console.log(fields);
      const notResults = _.map(
        results[items[activeIndex]?.label]?.data?.hasNot,
        "remarks"
      ).map((r, i) => {
        return {
          row: results[items[activeIndex]?.label]?.data?.hasNot[i][fields[0]],
  ...
  const uploader = async () => {
    ...
    const serviceLabels = service.schemaList.map((field) => {
      if(typeof field.reference.refServiceName !== 'undefined'){
        return { field: field.fieldName, label: field.reference.identifierFieldName };
      }
    ...
    <div className="card flex justify-content-center">
      ...
      >
        ...
        <Fieldset legend="Summary" toggleable>
          ...
          {hasNotResults.length !== 0 && (
            <p className="m-0">
              {hasNotResults.length === 1} Incompleted Field is not ready for
              upload
              {hasNotResults.length > 1} Incompleted Fields are not ready for
              upload
            </p>
          )}
        </Fieldset>
